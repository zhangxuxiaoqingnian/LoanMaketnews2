...
public class FragmentHelper<T> {
    ...
    public FragmentHelper(Context context, int containerId,
    ...
    /**
     * 添加Tab
     * @param positionId  Tab对应的菜单Id
     * @param tab Tab
     */
    public FragmentHelper<T> add(int positionId,Tab<T> tab){
        tabs.put(positionId,tab);
        return this;
    }
    ...
    public void setCurrentTab(int menuId){
        Tab<T> tab = tabs.get(menuId);
        if(currentTab!=null){
            currentTab = tab;
        }
    ...
    private void doTabChange(Tab<T> newTab,Tab<T> oldTab){
        ...
        if(newTab!=null){
            if(newTab.fragment==null){
                ...
                ft.replace(containerId,fragment,newTab.clx.getName());
            }else{
