...
public class MarkViewTwo extends LinearLayout implements View.OnClickListener {
    ...
    public MarkViewTwo(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    private void changeViewState(){
        for (int i = 0; i < 5; i++) {
            if((i + 1) * 2 <= mValue){
                imageViews[i].setImageResource(R.mipmap.pighong);
            } else if(i * 2 < mValue) {
                imageViews[i].setImageResource(R.mipmap.pighong);
            } else {
                imageViews[i].setImageResource(R.mipmap.pighuise);
            }
        }
    }
